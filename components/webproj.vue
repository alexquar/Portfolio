...
const props = defineProps<{ info: WebProject }>()
...
<template>
  <article data-aos="fade-up" class="glass-card tilt-card group overflow-hidden rounded-3xl p-6 sm:p-8 transition-all duration-500 hover:shadow-2xl hover:shadow-cyan-500/10">
    <div class="grid gap-10 md:grid-cols-[1.1fr_0.9fr] md:items-center">
      ...
      <div class="order-1 md:order-2 relative aspect-video overflow-hidden rounded-2xl border border-white/10 bg-slate-900/50 shadow-2xl">
        <img :src="props.info.image.src" :alt="props.info.image.alt" class="h-full w-full object-cover transition-transform duration-700 group-hover:scale-110" />
        <div class="absolute inset-0 bg-gradient-to-t from-slate-950/60 to-transparent opacity-0 transition-opacity duration-500 group-hover:opacity-100"></div>
      </div>
    </div>
  </article>
</template>
